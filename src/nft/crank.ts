...
const getKeypair = (secretKeyFilePath: String): Keypair => {
  return anchor.web3.Keypair.fromSecretKey(
    new Uint8Array(
      JSON.parse(require("fs").readFileSync(secretKeyFilePath, "utf8"))
    )
  );
};
...
const callMinter = async (): Promise<[StringPublicKey | undefined, string]> => {
  ...
  const walletAddressPath: string =
    process.env.PROGRAM_WALLET == undefined
      ? "../../env/program_wallet.json"
      : process.env.PROGRAM_WALLET;
  ...
  const cluster: string =
    process.env.DEV_NET == undefined
      ? "http://localhost:8899"
      : process.env.DEV_NET;

  const imagesPath: string =
    process.env.IMAGES_FOLDER == undefined
      ? "../images"
  ...
  const metaDataAccount: any = await mintNFT(
    provider.connection,
    walletWrapper,
    "local_net",
    files[0],
