...
export const getAssetCostInWinston = async (
  imageBuffer: Buffer,
  metadataJson: Buffer
) => {
  ...
  const txnFeeInWinstons = parseInt(
    (await axios.get("https://arweave.net/price/0")).data
  );
  console.log("txn fee", txnFeeInWinstons);
  const byteCostInWinstons = parseInt(
    (await axios.get("https://arweave.net/price/" + totalBytes.toString())).data
  );
  console.log("byte cost", byteCostInWinstons);
  const totalArCost =
    (txnFeeInWinstons * transBytes + byteCostInWinstons) / WINSTON_MULTIPLIER;
